...
VALUES(3, 14944, (SELECT PRECIO FROM PRODUCTO WHERE IDPRODUCTO = 14944) * 3, 3);

--INSERTAR PRODUCTO VENDIDO 2
INSERT INTO PROJECT.PROD_VENDIDO
(IDFACTURA, IDPRODUCTO, PRECIO, CANTIDAD)
VALUES(3, 14470, (SELECT PRECIO FROM PRODUCTO WHERE IDPRODUCTO = 14470) * 5, 5);

--INSERTAR PRODUCTO VENDIDO 3
INSERT INTO PROJECT.PROD_VENDIDO
(IDFACTURA, IDPRODUCTO, PRECIO, CANTIDAD)
VALUES(3, 10471, (SELECT PRECIO FROM PRODUCTO WHERE IDPRODUCTO = 10471) * 4, 4);

--ACTUALIZAR EL IMPORTE Y EL IVA DE LA FACTURA BASADO EN LA SUMA DE LOS PRECIOS
UPDATE PROJECT.FACTURA
SET IVA=(SELECT SUM(PRECIO)* 0.16 FROM PROD_VENDIDO WHERE IDFACTURA = 3),
IMPORTE=(SELECT SUM(PRECIO)* 1.16 FROM PROD_VENDIDO WHERE IDFACTURA = 3)
WHERE IDFACTURA=3;

--REVISAR QUE SE A?ADIO CORRECTAMENTE
SELECT * FROM FACTURA WHERE IDFACTURA = 3;

SELECT * FROM PROD_VENDIDO pv WHERE IDFACTURA = 3;
