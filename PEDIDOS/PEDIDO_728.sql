--SELECT PARA IDENTIFICAR LOS PRODUCTOS QUE PUEDE SURTIR EL PROOVEDOR 1
SELECT p.IDPROVEEDOR, p.NOMBRE, ps.IDPRODUCTO AS SURTE_PRODUCTO FROM  PROVEEDOR p, PROD_SURTIDO ps
WHERE p.IDPROVEEDOR = ps.IDPROVEEDOR AND p.IDPROVEEDOR = 5;

--INSERTAR EL PEDIDO
INSERT INTO PROJECT.PEDIDO
(IDPEDIDO, FECHA_PEDIDO, IMPORTE, FECHA_RECIBIDO, IVA, IDPROVEEDOR)
VALUES(728, '2012-03-25-00.00.00.000000', 1059.66, '2012-03-26-00.00.00.000000', 146.16, 5);

--REVISAR QUE SE INSERTO CORRECTAMENTE
SELECT * FROM PEDIDO WHERE IDPEDIDO = 728;

--INSERTAR LOS PRODUCTOS PEDIDOS

--SELECT PARA CALCULAR EL COSTO PRODUCTO 1
SELECT PRECIO, PRECIO / 1.3 AS COSTO FROM PRODUCTO WHERE IDPRODUCTO = 15926;

--INSERT PRODUCTO 1
INSERT INTO PROJECT.PROD_PEDIDO
(IDPRODUCTO, IDPEDIDO, CANTIDAD, COSTO)
VALUES(15926, 728, 3, 112);

--SELECT PARA CALCULAR EL COSTO PRODUCTO 2
SELECT PRECIO, PRECIO / 1.3 AS COSTO FROM PRODUCTO WHERE IDPRODUCTO = 20156;

--INSERT PRODUCTO 2
INSERT INTO PROJECT.PROD_PEDIDO
(IDPRODUCTO, IDPEDIDO, CANTIDAD, COSTO)
VALUES(20156, 728, 9, 35);

--SELECT PARA CALCULAR EL COSTO PRODUCTO 3
SELECT PRECIO, PRECIO / 1.3 AS COSTO FROM PRODUCTO WHERE IDPRODUCTO = 20614;

--INSERT PRODUCTO 3
INSERT INTO PROJECT.PROD_PEDIDO
(IDPRODUCTO, IDPEDIDO, CANTIDAD, COSTO)
VALUES(20614, 728, 5, 52.5);

--REVISAR QUE SE INSERTARON CORRECTAMENTE
SELECT * FROM PROD_PEDIDO pp WHERE IDPEDIDO = 728;
